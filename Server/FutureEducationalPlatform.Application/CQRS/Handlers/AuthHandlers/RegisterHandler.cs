...
namespace FutureEducationalPlatform.Application.CQRS.Handlers.AuthHandlers
{
    public class RegisterHandler : IRequestHandler<RegisterRequest, string>
    {
        ...
        {
            if (await _userService.GetByUserNameAsync(request.CreateUserDto.UserName) != null|| await _userService.GetByEmailAsync(request.CreateUserDto.Email) != null) throw new BadRequestException("البريد الالكتروني او اسم المستخدم موجود بالفعل");
            ...
            return "تم ارسال رمز التحقق الي بريدك الالكتروني";
        }
    }
}
